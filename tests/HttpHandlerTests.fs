...
let ``GET "/jsonChunked" returns json object`` (size: int, settings) =
    ...
    let app =
        GET |> HttpHandler.choose [
            route "/"     |> HttpHandler.text "Hello World"
            route "/foo"  |> HttpHandler.text "bar"
            route "/jsonChunked" |> HttpHandler.json ( Array.replicate size { Foo = "john"; Bar = "doe"; Age = 30 } )
            setStatusCode 404 |> HttpHandler.text "Not found" ]
    ...
    let app =
        choose [
            GET |> HttpHandler.choose [
                ...
                route "/foo"  |> HttpHandler.text "bar" ]
            POST |> HttpHandler.choose [
                route "/text"   |> HttpHandler.mustAccept [ "text/plain" ] |> HttpHandler.text "text"
                route "/json"   |> HttpHandler.mustAccept [ "application/json" ] |> HttpHandler.json "json"
                route "/either" |> HttpHandler.mustAccept [ "text/plain"; "application/json" ] |> HttpHandler.text "either" ]
            setStatusCode 404 |> HttpHandler.text "Not found" ]
