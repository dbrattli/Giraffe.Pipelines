    ...
    let app =
        ...
        |> HttpHandler.choose
            [ route "/" |> HttpHandler.text "Hello World"
              route "/foo" |> HttpHandler.text "bar"
              route "/jsonChunked"
              |> HttpHandler.json (Array.replicate size { Foo = "john"; Bar = "doe"; Age = 30 })
              setStatusCode 404 |> HttpHandler.text "Not found" ]
            ...
            [ GET
              ...
              |> HttpHandler.choose
                  [ route "/post/1" |> HttpHandler.text "1"
                    route "/post/2" |> HttpHandler.text "2" ]
              setStatusCode 404 |> HttpHandler.text "Not found" ]
    ...
    let app =
        choose
            [ GET
              ...
              |> HttpHandler.choose
                  [ route "/text"
                    |> HttpHandler.mustAccept [ "text/plain" ]
                    |> HttpHandler.text "text"
                    route "/json"
                    |> HttpHandler.mustAccept [ "application/json" ]
                    |> HttpHandler.json "json"
                    route "/either"
                    |> HttpHandler.mustAccept [ "text/plain"; "application/json" ]
                    |> HttpHandler.text "either" ]
              setStatusCode 404 |> HttpHandler.text "Not found" ]
    ...
    let personView model =
        html
            ...
            [ head [] [ title [] [ str "Html Node" ] ]
              body [] [ p [] [ sprintf "%s %s is %i years old." model.Foo model.Bar model.Age |> str ] ] ]
    ...
    let app =
        choose
            [ GET
              |> HttpHandler.choose
                  [ route "/" |> HttpHandler.text "Hello World"
                    route "/person" |> (personView johnDoe |> HttpHandler.htmlView) ]
              POST |> HttpHandler.choose [ route "/post/1" |> HttpHandler.text "1" ]
              setStatusCode 404 |> HttpHandler.text "Not found" ]
...
let ``Warbler function should execute inner function each time`` () =
    ...
    let app =
        GET
        |> HttpHandler.choose
            [ route "/foo" |> HttpHandler.text (inner ())
              route "/foo2" |> HttpHandler.warbler (fun _ -> text (inner ())) ]
        <| next
...
let ``GET "/redirect" redirect to "/" `` () =
    ...
    let app =
        ...
        |> HttpHandler.choose
            [ route "/" |> HttpHandler.text "Hello World"
              route "/redirect" |> HttpHandler.redirectTo false "/"
              setStatusCode 404 |> HttpHandler.text "Not found" ]
...
let ``POST "/redirect" redirect to "/" `` () =
    ...
    let app =
        ...
        |> HttpHandler.choose
            [ route "/" |> HttpHandler.text "Hello World"
              route "/redirect" |> HttpHandler.redirectTo true "/"
              setStatusCode 404 |> HttpHandler.text "Not found" ]
...
let negotiationTestApp =
    ...
    |> HttpHandler.choose
        [ route "/" |> HttpHandler.text "Hello World"
          route "/foo" |> HttpHandler.text "bar"
          route "/auto" |> HttpHandler.negotiate johnDoe
          setStatusCode 404 |> HttpHandler.text "Not found" ]
...
let ``Get "/auto" with Accept header of "application/json" returns JSON object``
    ...
    let ctx =
        getNegotiationTestHttpContext
            ...
            (Some(Newtonsoft None))
            false
            (StringValues("application/json"))
...
let ``Get "/auto" with Accept header of "application/xml" returns XML object``
    ...
    =
    let ctx =
        getNegotiationTestHttpContext config.NegotiationConfig None true (StringValues("application/xml"))
...
let ``Get "/auto" with Accept header of "application/xml, application/json" returns XML object``
    ...
    let ctx =
        getNegotiationTestHttpContext
            ...
            (Some(Newtonsoft None))
            true
            (StringValues("application/xml, application/json"))
...
let ``Get "/auto" with Accept header of "application/json, application/xml" returns JSON object``
    ...
    let ctx =
        getNegotiationTestHttpContext
            ...
            (Some(Newtonsoft None))
            false
            (StringValues("application/json, application/xml"))
...
let ``Get "/auto" with Accept header of "text/html" returns a 406 response``
    ...
    =
    let ctx =
        getNegotiationTestHttpContext config.NegotiationConfig None false (StringValues("text/html"))
...
let ``Get "/auto" without an Accept header returns a JSON object`` (config: NegotiationConfigWithExpectedResult) =
    let ctx =
        getNegotiationTestHttpContext config.NegotiationConfig (Some(Newtonsoft None)) false StringValues.Empty
