...
let mockJson (ctx: HttpContext) (settings: MockJsonSettings) =
    ...
    match settings with
    | Newtonsoft settings ->
        let jsonSettings = defaultArg settings NewtonsoftJson.Serializer.DefaultSettings
    ...
    | Utf8 settings ->
        let resolver = defaultArg settings Utf8Json.Serializer.DefaultResolver
    ...
    | SystemTextJson settings ->
        let jsonOptions = defaultArg settings SystemTextJson.Serializer.DefaultOptions
...
type JsonSerializersData =
    ...
    static member DefaultSettings = [ Utf8 None; Newtonsoft None; SystemTextJson None ]
